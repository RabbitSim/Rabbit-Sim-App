...
const dayNumHelper = (dayNumParam?: number, color?: string) => {
  ...
  const r = Math.min(
    255,
    Math.max(0, Math.floor(parseInt(color.slice(1, 3), 16) * mult))
  );
  const g = Math.min(
    255,
    Math.max(0, Math.floor(parseInt(color.slice(3, 5), 16) * mult))
  );
  const b = Math.min(
    255,
    Math.max(0, Math.floor(parseInt(color.slice(5, 7), 16) * mult))
  );
  const dimmedColor = `#${r.toString(16).padStart(2, "0")}${g
    .toString(16)
...
const sudoColonyRefs = useRef<sudoColony[]>([
    ...
    const downHandler = (e: KeyboardEvent) => {
      ...
      if (key === "s") {
        if (!sPressedRef.current) {
          ...
          for (let i = 0; i < rabbitCount; i++) {
            ...
            rabbitsRef1.current.push(
              new Rabbit(
                100 + Math.random() * 2,
                30 + Math.random() * 2,
                groupFoodCollectionGoals[0],
                "#ffffff"
              )
            );
          }
      ...
      if (key === "a") {
        if (!aPressedRef.current) {
          ...
          for (let i = 0; i < rabbitCount; i++) {
            ...
            rabbitsRef2.current.push(
              new Rabbit(
                60 + Math.random() * 2,
                40 + Math.random() * 2,
                groupFoodCollectionGoals[1],
                "#ff69b4"
              )
            );
          }
      ...
      if (key === "z") {
        if (!zPressedRef.current) {
          ...
          for (let i = 0; i < rabbitCount; i++) {
            ...
            rabbitsRef3.current.push(
              new Rabbit(
                30 + Math.random() * 2,
                15 + Math.random() * 2,
                groupFoodCollectionGoals[2],
                "#50c2e7ff"
              )
            );
          }
      ...
      if (key === "x") {
        if (!xPressedRef.current) {
          ...
          for (let i = 0; i < rabbitCount; i++) {
            ...
            rabbitsRef4.current.push(
              new Rabbit(
                10 + Math.random() * 2,
                62 + Math.random() * 2,
                groupFoodCollectionGoals[3],
                "#bbbb10ff"
              )
            );
          }
      ...
      if (key === "c") {
        if (!cPressedRef.current) {
          ...
          for (let i = 0; i < rabbitCount; i++) {
            ...
            rabbitsRef5.current.push(
              new Rabbit(
                110 + Math.random() * 2,
                70 + Math.random() * 2,
                groupFoodCollectionGoals[4],
                "#808080"
              )
            );
          }
      ...
      if (key === "f") {
        if (!fPressedRef.current) {
          ...
          foxesRef.current.push(
            new Fox(60 + Math.random() * 2, 40 + Math.random() * 2)
          );
        }
      }
      if (key === "k") {
  ...
  useEffect(() => {``
    ...
    const loop = () => {
      if (simulating) {
        ...
        // Combine all rabbits for fox hunting
        ...
        // Update foxes with all required parameters
        const worldWidth = 120; // Match your canvas world dimensions
        const worldHeight = 100;

        for (const fox of foxesRef.current) {
          const killedRabbits = fox.update(
            allRabbits,
            foxesRef.current,
            worldWidth,
            worldHeight
          ...
          // Add killed rabbits to dead rabbits display
          for (const killed of killedRabbits) {
            deadRabbitsRef.current.push(
              new DeadRabbit(killed.x, killed.y, 100)
            );
          }
        }
  ...
  useEffect(() => {
    if (foodStorageRef.current.length === 0) {
      foodStorageRef.current = burrowPositions.map(
        (pos) => new FoodStorage(pos.x + 4, pos.y + 1)
      );
    }
  ...
  const drawSprites = useCallback<Draw>(
    (ctx, frameCount, spritesArg) => {
