...
export class Attack implements IAction {
    ...
    takeAction(actor: Colony, target?: Colony, isDay?: boolean): void {
        ...
        if (power * (upset ? 0.7 : 1.0) > 1.1) {
            // attacker advantage
            const damage = Math.round(actor.population * 0.25 * power * critMult);
            targetLoss = damage;
            target.population = Math.max(0, target.population - targetLoss);

            // Loot some food as spoils of war
            const loot = Math.round(target.foodStorage * 0.3 * Math.random());
            ...
            // Near-even fight
            const damage = Math.round(actor.population * 0.15 * (0.8 + Math.random() * 0.4));
            targetLoss = damage;
            actorLoss = Math.round(damage * (0.8 + Math.random() * 0.5));
            target.population = Math.max(0, target.population - targetLoss);
            actor.population = Math.max(0, actor.population - actorLoss);

            // shared unrest spike
            actor.unrest = ColonyMath.clamp(actor.unrest + 0.08, 0, 1);
            target.unrest = ColonyMath.clamp(target.unrest + 0.08, 0, 1);

            log = `${actor.name} fought ${target.name} to a brutal draw. ${actorLoss} attackers and ${targetLoss} defenders perished.`;

        } else {
            // Defender advantage
            const damage = Math.round(actor.population * 0.2 * (0.8 + Math.random() * 0.5));
            actorLoss = damage;
            actor.population = Math.max(0, actor.population - damage);

            // morale collapse chance
            if (Math.random() < 0.35) {
                actor.unrest = ColonyMath.clamp(actor.unrest + 0.15, 0, 1);
